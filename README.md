...
Portable **Agent Skills** playbooks for `agents code-review` / `agents triage` workflows live under [`docs/skills/`](docs/skills/README.md). Install them with **`agents skills install <id>`** (see **`agents skills --help`**) into your host’s skills directory (for example **`~/.agents/skills/<id>/`**) when you want that guidance in Cursor, Claude Code, or another compatible client. Published **`@aguil/agents`** tarballs include **`docs/skills/`** so the same commands work after `npm install -g`.
