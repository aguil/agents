 ...
 * package's dependency after `npm install`. Resolve that first, then
 * BUN_INSTALL / PATH, then re-exec the Bun-targeted bundle (import.meta.main).
 * A candidate only wins if it is an executable file and not the bun package's
 * pre-postinstall placeholder, so a broken dependency never shadows a real Bun.
 */
export function launcherScript(bundleFile: string): string {
  return `${[
    ...
    "",
    "// The bun npm package ships bin/bun.exe on every platform as a small shell",
    "// placeholder; its postinstall swaps in the real binary. Installers that",
    "// skip scripts (--ignore-scripts, pnpm, aube) leave the placeholder behind.",
    'const bunPlaceholderMarker = "postinstall script was not run";',
    "const bunPlaceholderMaxBytes = 4096;",
    "let bunPlaceholderDir = null;",
    "",
    "function isBunPlaceholder(candidate, size) {",
    "  if (size > bunPlaceholderMaxBytes) {",
    "    return false;",
    "  }",
    "  try {",
    '    return readFileSync(candidate, "utf8").includes(bunPlaceholderMarker);',
    "  } catch {",
    "    return false;",
    "  }",
    "}",
    ...
    "      return false;",
    "    }",
    "    if (isBunPlaceholder(candidate, stats.size)) {",
    "      bunPlaceholderDir = dirname(dirname(candidate));",
    "      return false;",
    "    }",
    ...
    "    // bin/bun.exe is the package's bin entry on macOS and Linux too.",
    ...
    "  );",
    "  if (bunPlaceholderDir !== null) {",
    "    console.error(",
    '      "The bun dependency was installed without its postinstall script, so it has no binary.",',
    "    );",
    '    console.error("Run it manually: cd " + bunPlaceholderDir + " && node install.js");',
    "  } else {",
    "    console.error(",
