...
1. **Generic outcome type** (`packages/core`): `HarnessOutcome`
   ...
   declare an `execution` config, so legacy result shape and I/O are unchanged.

2. **Execution modes** (`packages/orchestration`): `HarnessDefinition.execution`
   selects `parallel` (default when absent — existing fan-out), `chain`
   (sequential; `{previous}` / `{outputs.<roleId>}` prompt interpolation; abort
   at first failed step), or `validation-loop` (implementation roles in parallel
   → validators via `{previous}`; retry with `{validation}` feedback up to
   `maxRounds`; pass condition defaults to "validators emitted zero outcomes" as
   a build-time predicate until an expression language lands). Inter-step output
   is truncated at 2000 lines / 50KB (limits matching pi-subagents) to prevent
   context overflow.
...
3. **Generic context** (`packages/context`): `ContextRequest.params` is the
   ...
   fields are deprecated but honored with precedence during migration. Generic
   providers `static-file`, `shell-command`, and `file-glob` cover fixture-style
   inputs. File-based providers enforce workspace containment by default
   (realpath-based, symlink-safe); escaping requires the explicit
...
4. **Worker registry** (`packages/workers`): `builtinWorkerHandlers()` registers
   `code_review` / `pr_feedback` / `implementation`;
