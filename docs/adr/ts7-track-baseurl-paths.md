# Track: `baseUrl` + `paths` before TypeScript 7

**Status:** Accepted (tracking item; work not done yet)  
**Context:** TypeScript 6.0 deprecates `compilerOptions.baseUrl` (TS5101); it will stop working in TypeScript 7. This monorepo uses `baseUrl` + `paths` in the root [`tsconfig.json`](../../tsconfig.json) for `@aguil/agents-*` workspace aliases.
