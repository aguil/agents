...
Review the context bundle for correctness, reliability, and maintainability
issues with behavioral impact.

Do not emit nitpicks. Prefer approval when no verified bug exists. Prefer
findings directly grounded in the provided diff/context artifacts. If repository
commands are needed, prefer the workspace VCS mode guidance from the run
request.
