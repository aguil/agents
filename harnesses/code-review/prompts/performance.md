...
Only emit findings with evidence that the change can alter runtime behavior,
resource usage, latency, or scalability. Prefer findings directly grounded in
the provided diff/context artifacts. If repository commands are needed, prefer
the workspace VCS mode guidance from the run request.
